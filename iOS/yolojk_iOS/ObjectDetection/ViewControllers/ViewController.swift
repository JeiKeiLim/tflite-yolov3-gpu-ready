...
extension ViewController: CameraFeedManagerDelegate {
  ...
  @objc  func runModel(onPixelBuffer pixelBuffer: CVPixelBuffer) {
    ...
    DispatchQueue.main.async {
      ...
      var postProcessingTime: Double = 0
      if let resultInferenceTime = self.result?.inferenceTime {
        inferenceTime = resultInferenceTime
      }
      if let resultPPTime = self.result?.postProcessingTime {
        postProcessingTime = resultPPTime
      }
      self.inferenceViewController?.inferenceTime = inferenceTime
      self.inferenceViewController?.postProcessingTime = postProcessingTime
