...
class InferenceViewController: UIViewController {
  ...
  var fps: Int {
    get {
      guard (inferenceTime + postProcessingTime) > 0 else {
        return 0
      }
      return Int(1000 / (inferenceTime + postProcessingTime))
    }
  }
...
extension InferenceViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func displayStringsForInferenceInfo(atRow row: Int) -> (String, String) {
    ...
    case .PostProcessingTime:
      info = String(format: "%.2fms", postProcessingTime)
    case .FPS:
