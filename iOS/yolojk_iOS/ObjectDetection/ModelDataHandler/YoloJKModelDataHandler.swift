...
protocol ScaledImageDelegate: class {
...
class YoloJKModelDataHandler: NSObject {
  // MARK: - delegate
  weak var scaledImageDelegate: ScaledImageDelegate?
  ...
  private let yoloResult: YoloJKResult
...
//    metalOptions.allowsPrecisionLoss = true
    ...
    self.yoloResult = YoloJKResult(inputWidth: inputWidth, inputHeight: inputHeight, labels: labels)
  ...
  func runModel(onFrame pixelBuffer: CVPixelBuffer) -> Result? {
    ...
    yoloResult.srcSize = YoloJKResult.Size(width: imageWidth, height: imageHeight)
    ...
    scaledImageDelegate?.didOutput(pixelBuffer: scaledPixelBuffer)
    ...
    let resultArray = yoloResult.bboxesToInferences(yoloBoxes: yoloBoxes)
    ...
    let result = Result(inferenceTime: inferenceInterval, postProcessingTime: postInterval, inferences: resultArray)
    return result
  }
