...
class YoloJKResultDecoder: NSObject {
  struct Size {
    var width: Int
    var height: Int
    var size: Int {
  ...
  func bboxesToInferences(yoloBoxes: [YoloJKBox], sortByBoxSize: Bool = true) -> [Inference] {
...
//                                                    y: CGFloat(self.srcSize!.height)))
    ...
    if sortByBoxSize {
      resultArray.sort { (first, second) -> Bool in
        return (first.rect.size.width * first.rect.size.height) > (second.rect.size.width * second.rect.size.height)
      }
    } else {
      resultArray.sort { (first, second) -> Bool in
