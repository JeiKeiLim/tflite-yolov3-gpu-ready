...
class InferenceViewController: UIViewController {
  ...
  var preProcessingTime: Double = 0
  var postProcessingTime: Double = 0
  var fps: Int {
    get {
      guard (inferenceTime + postProcessingTime + preProcessingTime) > 0 else {
        return 0
      }
      return Int(1000 / (preProcessingTime + inferenceTime + postProcessingTime))
    }
  }
...
extension InferenceViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func displayStringsForInferenceInfo(atRow row: Int) -> (String, String) {
    ...
    case .PostProcessingTime:
      info = String(format: "%.2fms, %.2fms", preProcessingTime, postProcessingTime)
    case .FPS:
      info = String(format: "%02d", fps)
    }
