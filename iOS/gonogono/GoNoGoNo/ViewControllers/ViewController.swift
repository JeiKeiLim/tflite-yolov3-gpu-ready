...
class ViewController: UIViewController {
  ...
  @IBOutlet weak var gonogoLabel: UILabel!
  @IBOutlet weak var carStateLabel: UILabel!
  
  // MARK: Constants
  private let displayFont = UIFont.systemFont(ofSize: 11.5, weight: .medium)
  ...
  private var modelDataHandler: YoloJKModelDataHandler? =
    YoloJKModelDataHandler(modelFileInfo: YoloJK.modelInfo, labelsFileInfo: YoloJK.labelsInfo)
  private var gonogono: GoNoGoNo = GoNoGoNo()
  private var inferenceViewController: InferenceViewController?
  
  private var player: AVAudioPlayer?
  
  // MARK: Running Variables
  struct InferenceDelay {
    var inferTime: Double = 0
    var delayAlpha: Double
    var delayBetweenInferencesMs: Double {
      get {
        inferTime * delayAlpha
      }
    }
  }
  var inferenceDelay = InferenceDelay(delayAlpha: 1.0)

  // MARK: View Handling Methods
  override func viewDidLoad() {
    super.viewDidLoad()
    
    if let modelHandler = modelDataHandler {
      modelHandler.gonogonoDelegate = gonogono
      modelHandler.gonogonoDelegate?.labelInfoArrived(labels: modelHandler.labels)
    } else {
      fatalError("Failed to load model")
    }
    
    gonogono.delegate = self
    cameraFeedManager.delegate = self
    overlayView.clearsContextBeforeDrawing = true
    debugLabel.isHidden = true
    gonogoLabel.isHidden = true
    carStateLabel.text = "Go"
    
    let tapGesture = UITapGestureRecognizer(target: self, action: #selector(onTouchTopOverlay(_:)))
    topOverlayView.addGestureRecognizer(tapGesture)
    addPanGesture()
  }
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "EMBED" {
      ...
      inferenceViewController?.postProcessingTime = tempResult.postProcessingTime
...
extension ViewController {
  func playSound(name: String, ext: String = "mp3") {
      guard let url = Bundle.main.url(forResource: name, withExtension: ext) else {
        print("Can't open file \(name).\(ext)")
        return
    }
...
// MARK: GoNoGoNo State Delegate
extension ViewController: GoNoGoNoStateDelegate {
  func go(state: GoNoGoNoState) {
    var state = state
    let isMoving = state.isCarMoving
    var isHidden = true

    DispatchQueue.main.sync {
      isHidden = self.gonogoLabel.isHidden
    }
    
    guard isHidden == true && isMoving == false else {
      return
    }
    
//    let lastGoTime = state.lastGoTime
    let isCarGo = state.frontCarGo
    
    if isMoving == false {
      DispatchQueue.main.async {
        self.gonogoLabel.text = isCarGo ? "GO!!" : "GO!"
        self.gonogoLabel.isHidden = false
        
        if let player = self.player {
          if player.isPlaying == false {
            self.playSound(name: "singingbowl01")
          }
        } else {
          self.playSound(name: "singingbowl01")
        }
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
        self.gonogoLabel.isHidden = true
      }
    }
  }
  ...
  func no(state: GoNoGoNoState) {
    ...
    DispatchQueue.main.async {
      self.carStateLabel.text = state.isCarMoving ? "Go" : "Stop"
      var debugMsg = String(format: "Car: %.1f%% (%.1f)", state.gonogo*100, state._dbgDiffValue) + ", \(state._dbgMsg)\n"
      debugMsg += String(format: "Scores: %.1f (%.1f), %.1f (%.1f, %.1f, %.1f)\n", state.boxScore, state._dbgBoxSize, state.tlScore, state._dbgTLRgb[0], state._dbgTLRgb[1], state._dbgTLRgb[2])
      debugMsg += String(format: "dTime: %03dms, bTime: %03dms", Int(state._dbgDriveCheckTime*1000), Int(state._dbgBoxProcessTime*1000))
...
extension ViewController: InferenceViewControllerDelegate {
  ...
  func didChangeThreadCount(to count: Int) {
    ...
    modelDataHandler = YoloJKModelDataHandler(
      modelFileInfo: YoloJK.modelInfo,
      labelsFileInfo: YoloJK.labelsInfo,
      threadCount: count
    )
  }
...
extension ViewController: CameraFeedManagerDelegate {
  ...
  func didOutput(pixelBuffer: CVPixelBuffer) {
    self.modelDataHandler?.gonogonoDelegate?.cameraImageArrived(pixelBuffer: pixelBuffer)
    
    if gonogono.state.isCarMoving == false {
  ...
  @objc  func runModel(onPixelBuffer pixelBuffer: CVPixelBuffer) {
    ...
    if self.modelDataHandler!.isRunning {
      previousInferenceTimeMs = currentTimeMs
      return
    }

    guard  (currentTimeMs - previousInferenceTimeMs) >= inferenceDelay.delayBetweenInferencesMs else {
      return
    }
    ...
    if let inferTime = result?.inferenceTime {
      inferenceDelay.inferTime = inferTime
    }
    ...
    DispatchQueue.main.async {
      ...
      if let r = self.result {
        self.inferenceViewController?.inferenceTime = r.inferenceTime
        self.inferenceViewController?.preProcessingTime = r.preProcessingTime
        self.inferenceViewController?.postProcessingTime = r.postProcessingTime
...
//      var convertedRect = inference.rect.applying(CGAffineTransform(scaleX: (self.overlayView.bounds.size.width / imageSize.width) * imageSize.width, y: (self.overlayView.bounds.size.height / imageSize.height) * imageSize.height ))
      var convertedRect = inference.correctBox(width: (self.overlayView.bounds.size.width / imageSize.width) * imageSize.width, height: (self.overlayView.bounds.size.height / imageSize.height) * imageSize.height )
...
// MARK: Custom Interaction Methods
extension ViewController {
  @objc func onTouchTopOverlay(_ sender: UITapGestureRecognizer) {
